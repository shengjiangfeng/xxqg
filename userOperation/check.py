...
def check_task(browser: XuexiChrome):
    ...
    table = Table(show_header=True, header_style="bold black",title_justify='center')
    table.add_column("每日登录", justify='center')
    table.add_column("选读文章", justify='center')
    table.add_column("视听学习", justify='center')
    table.add_column("每日答题", justify='center')
    table.add_column("今日累积积分", justify='center')
    ...
    if settings['观看视频'] == "true" and (scores[2].text != '12分/12分' ):
    ...
    if settings['每日答题'] == 'true' and res == CheckResType.NULL and task_buttons[3].text != '已完成':
    ...
    special_exam_path = './data/special_exam.json'
    date = datetime.now().strftime('%Y%m%d')
    # judge file exists
    with open(special_exam_path, 'r', encoding='utf-8') as f:
        special_exam = f.read()
    special_exam = loads(special_exam)
    # 专项答题
    if exam_temp['SPECIAL_EXAM'] == 'true' and date not in special_exam and settings['专项答题'] == 'true' and res == CheckResType.NULL:
    ...
    # if exam_temp['WEEKLY_EXAM'] == 'true' and settings['每周答题'] == 'true' and res == CheckResType.NULL and  len(task_buttons) >=6 and  task_buttons[5].text != '已完成':
