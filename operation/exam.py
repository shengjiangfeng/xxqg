...
def to_exam(browser: XuexiChrome, exam_type: check.CheckResType):
    ...
    if exam_type == check.CheckResType.DAILY_EXAM:
        ...
        browser.xuexi_get('https://pc.xuexi.cn/points/exam-paper-list.html')
        sleep(round(uniform(1, 2), 2))
...
def run_exam(browser: XuexiChrome, exam_type: check.CheckResType):
    while True:
        ...
        if question_index == question_count:
            ...
            try:
                ...
                alert.accept()
            print('--> 答题结束')
            if exam_type ==  check.CheckResType.SPECIAL_EXAM :
                ...
                special_exam[date] = 'true'
                with open(special_exam_path, 'w', encoding='utf-8') as f:
                    f.write(dumps(special_exam, ensure_ascii=False, indent=4))
                print("")
            break
...
def run_special_exam(browser: XuexiChrome):
    while True:
        content = browser.find_element(by=By.CLASS_NAME, value='ant-breadcrumb')
        browser.execute_script('arguments[0].scrollIntoView();', content)
        sleep(round(uniform(2, 3), 2))
